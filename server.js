/**
 * CreatorLens — Final server
 *
 * WHY 3 ENDPOINTS INSTEAD OF 1:
 * Railway free tier kills HTTP connections after ~100s.
 * Apify scrapes take up to 180s. A single long-polling endpoint
 * gets cut dead. Solution: the client polls with short requests.
 *
 * Flow:
 *   POST /start        { username, apifyKey }  → { runId, datasetId }   (~1s)
 *   GET  /status/:runId?apifyKey=xxx            → { status }             (~1s)
 *   GET  /dataset/:datasetId?apifyKey=xxx       → { data: [...] }        (~2s)
 *   GET  /health                                → { ok: true }
 *   GET  /                                      → full HTML app
 */
...
app.post("/start", async (req, res) => {
  ...
  if (!username || !apifyKey)
    return res.status(400).json({ error: "username and apifyKey required." });
  try {
    ...
    if (!r.ok) {
      const msg = b?.error?.message || b?.message || JSON.stringify(b).slice(0, 200);
      return res.status(r.status).json({ error: `Apify error: ${msg}` });
    }
    const runId = b?.data?.id;
    const datasetId = b?.data?.defaultDatasetId;
    if (!runId) return res.status(500).json({ error: "Apify gave no runId.", raw: b });
...
// ─── 2. Poll run status (called repeatedly by client every 5s) ───────────────
app.get("/status/:runId", async (req, res) => {
  const { apifyKey } = req.query;
  if (!apifyKey) return res.status(400).json({ error: "apifyKey required." });
  try {
...
// ─── 3. Fetch dataset once run SUCCEEDED ─────────────────────────────────────
app.get("/dataset/:datasetId", async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) return res.status(r.status).json({ error: "Dataset fetch failed.", raw: items });
    if (!Array.isArray(items) || !items.length)
      return res.status(404).json({ error: "No data. Account may be private or username wrong." });
...
// ─── HTML app (inline so no static file needed) ───────────────────────────────
...
:root{--bg:#06060E;--card:#0C0C18;--border:#181830;--amber:#F59E0B;--indigo:#6366F1;--green:#10B981;--red:#EF4444;--pink:#EC4899;--teal:#14B8A6;--orange:#F97316;--text:#EEEEFA;--muted:#565672;--dim:#1E1E30}
...
.btn-sm{background:var(--amber);color:#06060E;padding:9px 18px;font-size:13px;width:auto}
...
<div id="s-setup" class="screen" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px">
  <div style="width:100%;max-width:500px">
    <div style="margin-bottom:28px">
      ...
      <p style="color:var(--muted);font-size:13px;line-height:1.75">Paste a public Instagram handle and your Apify key. Full scrape + AI analysis in one click.</p>
    </div>
  ...
  <div style="max-width:500px;width:100%;text-align:center">
    ...
    <div class="card" style="margin-bottom:20px;text-align:left">
      ...
      <div style="font-size:12px;color:var(--muted);line-height:1.9">
        · <b style="color:var(--text)">Apify key wrong/expired</b> — regenerate at console.apify.com<br>
        · <b style="color:var(--text)">Account is private</b> — only public profiles work<br>
        · <b style="color:var(--text)">Apify credits exhausted</b> — check usage dashboard<br>
        · <b style="color:var(--text)">Username typo</b> — verify the handle on Instagram
      </div>
    </div>
...
const STEP_LABELS=["Starting Apify run","Scraping Instagram (30–90s)","Processing data","Calculating metrics","Running AI analysis","Building report"];
...
function setStep(n,msg){
  ...
  document.getElementById("steps").innerHTML=STEP_LABELS.map((s,i)=>
    \`<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
      <div style="width:22px;height:22px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;background:\${i<n?"#10B981":i===n?"#F59E0B":"#1E1E30"};color:\${i<=n?"#06060E":"#565672"}">
        \${i<n?"✓":i+1}
      </div>
...
function parseUser(s){s=(s||"").trim();try{const u=new URL(s.startsWith("http")?s:"https://"+s);return u.pathname.split("/").filter(Boolean)[0]||"";}catch{return s.replace(/^@/,"").split(/[/?#]/)[0];}}
function fmt(n){if(n==null||isNaN(n))return"0";if(n>=1e6)return(n/1e6).toFixed(1)+"M";if(n>=1e3)return(n/1e3).toFixed(1)+"K";return Math.round(n).toLocaleString();}
function pType(t=""){t=t.toLowerCase();if(t.includes("video")||t==="reel")return"reel";if(t.includes("sidecar")||t.includes("album")||t==="carousel")return"carousel";return"static";}
...
function statCard(l,v,sub,col){col=col||"#F59E0B";return \`<div class="card" style="flex:1;min-width:120px"><div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px">\${l}</div><div class="mono" style="font-size:20px;color:\${col};font-weight:700;line-height:1.1">\${v}</div>\${sub?\`<div style="font-size:11px;color:var(--muted);margin-top:3px">\${sub}</div>\`:""}</div>\`;}
...
function buildMetrics(raw){
  ...
  const scored=rawP.map(x=>({...x,_v:gv(x),_sc:getScore(x,F),_er:getER(x,F),_type:pType(x.type||""),
  ...
  return{handle:p.username||"unknown",name:p.fullName||p.name||p.username||"",bio:p.biography||"",
...
function buildPrompt(m){
  return "You are a brand-side social media analyst vetting Instagram creators for a men's health brand (Man Matters — hair loss, testosterone, fitness).\\n\\n"+
    ...
    "FORMAT: "+m.fmt.reel.pct+"% Reels · "+m.fmt.carousel.pct+"% Carousels · "+m.fmt.static.pct+"% Static | POSTS: "+m.total+"\\n\\n"+
...
async function startAnalysis(){
  ...
  try{
    // Step 1: start run
    const startRes=await fetch("/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username,apifyKey})});
    const startBody=await startRes.json();
    if(!startRes.ok)throw new Error(startBody.error||"Start failed: "+startRes.status);
    const {runId,datasetId}=startBody;

    // Step 2: poll status (client-side loop — each request is <1s, no connection held open)
    ...
    while(["RUNNING","READY","ABORTING"].includes(status)){
      ...
      const pollRes=await fetch("/status/"+runId+"?apifyKey="+encodeURIComponent(apifyKey));
      const pollBody=await pollRes.json();
      status=pollBody.status||"UNKNOWN";
      if(["FAILED","ABORTED","TIMED-OUT"].includes(status))
        throw new Error("Apify run "+status+". Check console.apify.com for details.");
      if(elapsed>180)throw new Error("Scrape timed out after 3 minutes. Retry or check Apify dashboard.");
    }

    // Step 3: fetch dataset
    setStep(2,"Fetching scraped data…");
    const dataRes=await fetch("/dataset/"+datasetId+"?apifyKey="+encodeURIComponent(apifyKey));
    const dataBody=await dataRes.json();
    if(!dataRes.ok)throw new Error(dataBody.error||"Dataset fetch failed");
    const items=dataBody.data;
    if(!Array.isArray(items)||!items.length)throw new Error("No data returned. Account may be private.");

    // Step 4: metrics
    setStep(3,"Calculating metrics…");
    ...
    let ai;try{ai=JSON.parse(txt);}catch(e){throw new Error("AI returned malformed JSON — retry. "+e.message);}
...
function renderReport(d,ai){
  ...
  const fc=(ai?.verdict?.score||0)>=7?"#10B981":(ai?.verdict?.score||0)>=5?"#F59E0B":"#EF4444";
  let h="";
  ...
  h+=\`<div style="margin-bottom:28px">\${sh("Creator Overview")}<div class="flex wrap g10">
    \${statCard("Followers",fmt(d.followers))}
    \${statCard("Avg ER",d.avgER+"%",d.avgER>=3?"Strong":d.avgER>=1.5?"Average":"Weak",d.avgER>=3?"#10B981":d.avgER>=1.5?"#F59E0B":"#EF4444")}
    \${statCard("Avg Reel Views",fmt(d.avgViews))}
    \${statCard("Views/Follower",d.vfr+"%",d.vfr>=30?"Excellent":d.vfr>=10?"Good":"Weak",d.vfr>=30?"#10B981":d.vfr>=10?"#F59E0B":"#EF4444")}
    \${statCard("Cadence",d.cadence,d.total+" posts")}
  </div></div>\`;
  ...
  h+=\`<div style="margin-bottom:28px">\${sh("Top 5 Posts","#F59E0B")}\`;
  d.top5.forEach((p,i)=>{
